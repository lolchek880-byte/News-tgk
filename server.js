...
/* Невидимая шкала интереса/срочности новости от 1 до 10, которую сам
   ИИ проставляет каждой новости (читателям не показывается). Ниже этого
   порога новость считается "скучной" и не публикуется. Действует для
   всех источников. */
const INTEREST_THRESHOLD = parseInt(
  process.env.INTEREST_THRESHOLD || "4",
  10
);
...
function buildSettings() {
  return (
    ...
    `🎯 Фильтр скучных новостей: включён для всех источников (порог интереса ${INTEREST_THRESHOLD}/10)\n` +
...
15. Поле "interest" — оцени по шкале от 1 до 10, насколько новость важная,
    интересная и срочная для широкой аудитории новостного канала. Это
    внутренняя оценка, читателям она не показывается.
    Примеры:
    - "В Сызрани починят дорогу" — 1 балл (рядовая локальная новость).
    - "Кот застрял на дереве, спасатели помогли" — 1-2 балла.
    - "Подорожали продукты в одном регионе" — 3-4 балла.
    - "Крупная авария с пострадавшими" — 6-7 баллов.
    - "Трамп выехал в Москву для переговоров" — 8 баллов.
    - "Начались боевые действия / крупный теракт / отставка президента" —
      9-10 баллов.
    Ориентиры: 1-3 — рядовая/бытовая/проходная заметка без значимых
    последствий; 4-6 — заметная новость среднего масштаба; 7-10 — важное,
    срочное или резонансное событие федерального/международного уровня.
    Оценивай честно и по существу новости, а не по эмоциональности
    заголовка.
`;
...
async function processWithAI(
  ...
  if (
    ...
    /* Если ИИ не прислал число — не блокируем публикацию,
       считаем новость нейтрально интересной. */
    interest = 10;
  }
  ...
  return {
    ...
    /* Невидимая шкала интереса 1-10, которую проставил ИИ.
       Используется для фильтрации только у источников
       с filterInterest: true (см. FEEDS). Читателям не показывается. */
    interest,
  };
}
        ...
        for (
          ...
          */
          ...
          try {
            ...
            /*
               Фильтр "скучных" новостей по невидимой шкале интереса
               (1-10, её выставляет сам ИИ) — теперь включён для всех
               источников.
            */
            if (
              ai.interest <
